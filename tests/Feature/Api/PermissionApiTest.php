...
test('shoud return permissions page 2', function () {
...
test('shoud return permissions with total_per_page', function () {
...
describe('validations', function () {
    test('should validate create new permission', function () {
        postJson(route('permissions.store'), [], ['Authorization' => 'Bearer ' . $this->token])
            ->assertStatus(422)
            ->assertJsonValidationErrors(['name', 'description']);
    });
    test('should validate update new permission', function () {
        putJson(route('permissions.update', [$this->permission->id]), [], ['Authorization' => 'Bearer ' . $this->token])
            ->assertStatus(422)
            ->assertJsonValidationErrors(['name', 'description']);
    });
    test('should validate update new permission - with name less 3 characters', function () {
        putJson(route('permissions.update', [$this->permission->id]), [
            'name' => 'Do',
            'descriptions' => 'List permissions',
        ], ['Authorization' => 'Bearer ' . $this->token])
            ->assertStatus(422)
            ->assertJsonValidationErrors([
                'name' => trans('validation.min.string', ['attribute' => 'nome', 'min' => 3])
            ]);
    });
});
...
test('should return 404 when permission not found', function () {
    getJson(route('permissions.show', 'fake_id'), ['Authorization' => 'Bearer ' . $this->token])
        ->assertNotFound();
});
...
test('should return 404 when not update permission', function () {
